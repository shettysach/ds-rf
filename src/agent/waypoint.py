...
def parse_waypoint_command(text: str) -> WaypointCommand:
    ...
    if motion not in {"stand", "walk"}:
        raise ValueError("Command motion must be stand or walk")
    if motion == "stand":
        if waypoint is not None:
            raise ValueError("Stand command requires waypoint_2d=null")
        return WaypointCommand(motion="stand", waypoint_2d=None)

    if not isinstance(waypoint, list) or len(waypoint) != 2:
        raise ValueError("Walk command requires waypoint_2d=[x,y]")
    ...
    return WaypointCommand(motion="walk", waypoint_2d=(x, y))
